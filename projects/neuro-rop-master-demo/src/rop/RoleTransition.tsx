...
export const RoleTransition: React.FC = () => {
  ...
  const inT = interpolate(frame, [0, 24], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.bezier(0.2, 0.8, 0.2, 1)});
  const outT = interpolate(frame, [94, 120], [1, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.bezier(0.33, 0, 0.15, 1)});
  return (
    <AbsoluteFill style={{backgroundColor: '#122b4d', overflow: 'hidden'}}>
      ...
      <div style={{position: 'absolute', left: 0, right: 0, top: 365, textAlign: 'center', color: '#fff', opacity: inT * outT, transform: `translateY(${(1 - inT) * 18}px)`}}>
        ...
        <div style={{width: 250, height: 4, margin: '30px auto 0', borderRadius: 4, background: '#4e8cf0', transformOrigin: 'center', transform: `scaleX(${interpolate(frame, [20, 48], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.bezier(0.2, 0.8, 0.2, 1)})})`}} />
      </div>
    </AbsoluteFill>
  );
};
